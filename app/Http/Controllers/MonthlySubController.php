...
class MonthlySubController extends Controller
{
    ...
    public function moexTopUpAccount($country, $amount, $name, $accountType)
    {
        try {
            ...
            $receiverMail = env('APP_ENV') === 'local' ? 'adenugaadebambo41@gmail.com' : 'moneyexchange@paysprint.ca';
            ...
            $activity = "Received " . $myaccount->currencyCode . '' . number_format($amount, 2) . " for ps-moex transaction from {$name}, with {$accountType} account to your FX Wallet.";
            $credit = $amount;
            $debit = 0;
            $reference_code = $transaction_id;
            $balance = 0;
