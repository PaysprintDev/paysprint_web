                                ...
                                @if (count($data['allAddedCards']) > 0)
                                    ...
                                    @foreach ($data['allAddedCards'] as $data)
                                        <tr>
                                            ...
                                            <td>{{ $data->bvn_account_name != null ? $data->bvn_account_name : ' - ' }}
                                            </td>
                                            <td>{{ $data->accountType }}</td>

                                            <td>
                                                <a href="{{ route('get user card', $data->user_id) }}" type="button"
                                                    class="btn btn-primary">View Cards</a>

                                                @if ($data->flagged == 0)
                                                    <a href="#" type="button" class="btn btn-danger"
                                                        onclick="flagAccount('{{ $data->user_id }}')">Flag Account
                                                        <img class="spin{{ $data->user_id }} disp-0"
                                                            src="https://i.ya-webdesign.com/images/loading-gif-png-5.gif"
                                                            style="width: 20px; height: 20px;"> <i
                                                            class="fas fa-flag"></i></a>
                                                @else
                                                    <a href="#" type="button" class="btn btn-success"
                                                        onclick="flagAccount('{{ $data->user_id }}')">Remove flag <img
                                                            class="spin{{ $data->user_id }} disp-0"
                                                            src="https://i.ya-webdesign.com/images/loading-gif-png-5.gif"
                                                            style="width: 20px; height: 20px;"> <i
                                                            class="fas fa-flag"></i></a>
                                                @endif
