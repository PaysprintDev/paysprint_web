...
<?php use \App\Http\Controllers\User; ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
         PaySprint Collection Report
      </h1>
      <ol class="breadcrumb">
      <li><a href="{{ route('Admin') }}"><i class="fa fa-dashboard"></i> Dashboard</a></li>
        <li class="active">PaySprint Collection Report</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">PaySprint Collection Report</h3> <br>
              <br>
              <div class="row">
                <div class="col-md-12">
            ...
            <div class="box-body">
              ...
              <table class="table table-bordered">
                <tbody>
                  ...
                  <tr>
                    <td style="color: black; font-weight: bold;">Total Amount Paid</td>
                ...
                <tbody id="statementtab">
                    ...
                    <?php $i = 1; $totAmount = 0;?>
                        ...
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ $items->name }}</td>
                            @if($user = \App\User::where('email', $items->email)->get())
                              @if(count($user) > 0)
                              ...
                              @endif
                            @endif
                            <td>{{ $items->payee_ref_no }}</td>
                            <td>{{ $items->invoice_no }}</td>
                            <td>{{ date('d/F/Y', strtotime($items->payment_due_date)) }}</td>
                            <td style="color: black; font-weight: bold;">{{ number_format($items->invoice_amount, 2) }}</td>
                            <td style="color: blue; font-weight: bold;">{{ number_format($items->opening_balance, 2) }}</td>
                            <td style="color: green; font-weight: bold;">{{ number_format($items->amount_paid, 2) }}</td>
                            <td style="color: red; font-weight: bold;">{{ number_format($items->invoice_amount - $items->amount_paid, 2) }}</td>
