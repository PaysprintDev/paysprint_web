				...
				<div class="card ">
					...
					<table class="table table-striped table-responsiveness">
						...
						<tbody>
							...
							<tr>
								<td>{{$counter++}}</td>
								<td>{{$reviews->name_of_sender}}</td>
								<td>{{$reviews->comment}}</td>
								<td><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop{{ $reviews->id }}">Reply Comment</button></td>
								<td><a href="{{route('view replies',$reviews->id)}}" class="btn btn-secondary">View Replies</a>
								<td>
								<td>
									@if ($reviews->status == 'pending')
									<p class="btn btn-danger">Pending Reply</p>
									@else
									<p class="btn btn-success">Replied</p>
									@endif
								</td>
							</tr>

							<!-- Modal -->
							<div class="modal fade" id="staticBackdrop{{ $reviews->id }}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
								<div class="modal-dialog">
									<div class="modal-content">
										<div class="modal-header">Marketplace Review</h5>
										</div>
										<div class="modal-body">
											<form action="{{route('merchant reply')}}" method="post">
												@csrf
												<div class="row">
													<div class="col-md-12">
														<div class="form-group">
															<label>Drop a Reply</label>
															<textarea class="form-control" style="width:100%; height:100px;" name="merchant_comment"></textarea>
														</div>
													</div>
