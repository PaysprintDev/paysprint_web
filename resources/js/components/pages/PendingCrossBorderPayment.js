...
class PendingCrossBorderPayment extends Component {
	_isMounted = false;
	...
	componentDidMount() {
		...
		try {
			axios
				.get(`/api/v1/getpendingcrossborderpayment`, { headers: { Authorization: `Bearer ${apiToken}` } })
				.then((res) => {
					if (this._isMounted) {
						if (res.status === 200) {
							...
							<div className="mb-2 mb-md-3 mb-xl-4 pb-2">
								<ul className="nav nav-tabs nav-tabs-md nav-tabs-line position-relative zIndex-0">
									<li className="nav-item">
										<Link className="nav-link " to={'/currencyfx/crossborderplatform'}>
											Processed Invoice Payment
										</Link>
									</li>
									<li className="nav-item">
										<Link className="nav-link active" to={'/currencyfx/pendingcrossborderpayment'}>
...
export default PendingCrossBorderPayment;
